...
(function ($) {
    ...
    $(document).ready(function () {
        ...
        // Initialize the role selection on page load (if already selected)
        $("input[id='role-1']").change(function () {
            var selectedRole = $("input[id='role-1']:checked").val();
            toggleInputs(selectedRole); // Call this function to toggle inputs when the role is selected
            // Hide error message when a role is selected
            $("#role-error-1").hide();
        });
        $("input[id='role-2']").change(function () {
            var selectedRole = $("input[id='role-2']:checked").val();
            toggleInputs(selectedRole); // Call this function to toggle inputs when the role is selected
            // Hide error message when a role is selected
            $("#role-error-2").hide();
        });
        $("input[id='role-3']").change(function () {
            var selectedRole = $("input[id='role-3']:checked").val();
            toggleInputs(selectedRole);
            $("#role-error-3").hide();
        });

        $(".next").click(function () {
            // Step-1 Validation for radio button selection (Seller or Buyer)
            if (current === 1) {
                var selectedRole = $("input[id='role-1']:checked").val();
                if (!selectedRole) {
                    // Show error message if no role is selected
                    $("#role-error-1").show();
                    return false; // Prevent form from going to the next step
                }
            }
    ...
    // Show/hide inputs based on Seller or Buyer selection
    function toggleInputs(role) {
        const sellerInputs = document.querySelectorAll('.seller-inputs');
        const buyerInputs = document.querySelectorAll('.buyer-inputs');

        if (role === 'seller') {
            sellerInputs.forEach(input => input.style.display = 'block');
            buyerInputs.forEach(input => input.style.display = 'none');
        } else if (role === 'buyer') {
            buyerInputs.forEach(input => input.style.display = 'block');
            sellerInputs.forEach(input => input.style.display = 'none');
        }
    ...
    // wishes popup image script start
    ...
    window.onload = function () {
        modal.style.display = "flex"; // Ensure modal shows as flex to center it
    };
    ...
    window.onclick = function (event) {
        if (event.target === modal) { // Close the modal when clicking outside
            modal.style.display = "none";
        }
    };


    // wishes popup image script end
})(jQuery);
